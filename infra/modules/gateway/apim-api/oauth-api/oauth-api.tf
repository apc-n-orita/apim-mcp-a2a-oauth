...
# apim-mcp-oauth の modules/core/gateway/apim-api/oauth-api を参考にした、
# OAuth 2.0 Protected Resource Metadata (RFC 9728) を公開する API。
# 既存 (apim-mcp-oauth 側) の oauth API は mcp-oauth-app の scope を返すが、
# こちらは toolbox API (audience=https://ai.azure.com/) 向けに ai.azure.com の scope を返す。
resource "azurerm_api_management_api" "oauth" {
  name                  = var.api_name
  resource_group_name   = var.resource_group_name
  api_management_name   = var.api_management_name
  revision              = "1"
  display_name          = "OAuth (Toolbox)"
  description           = "OAuth 2.0 Protected Resource Metadata for the Foundry Toolbox API (audience=https://ai.azure.com/)"
